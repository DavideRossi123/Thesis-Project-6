...
#!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!#
#NB data are collected with startified random sampling !!!!!!!!!!!!!!!!!!!!!!!!#
#!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!#

#Medits_Handbook_2017_version_9_5-60417r, this si the full handbook in which data
#specification are present, refer to it if necessary.

#Read "C:\Thesis\MEDITS survey specifications.pdf"!!!!!

#NB Medits uses bottom trawling and is about demersal fishes.
...
TA<-read_csv("C:\\Thesis\\Demersal\\TA.csv")
...
# This variable will be used to adjust the sign of the longitude coordinates for
# stations located east or west of the Greenwich meridian.
...
# Note that locations east of the Greenwich meridian are still stored with positive
...
TB<-read_csv("C:\\Thesis\\Demersal\\TB.csv")
...
#Similaryly to what we have done for shooting_longitude
...
#suggesting that month is a relevant covariate.
...
#Checking presence of spatial autocorrelation
MEDITS_MERL_MOD_LOG <- MEDITS_MERL_MOD_LOG %>%
  mutate(presence_num = ifelse(presence == "non zero", 1, 0))

#Put an image with the profiles of the countries on the background
ggplot(MEDITS_MERL_MOD_LOG, 
       aes(x = shooting_longitude, 
           y = shooting_latitude, 
           color = presence)) +
  geom_point(alpha = 0.7,size=0.1) +
  coord_fixed() +
  scale_color_manual(values = c("zero" = "red", "non-zero" = "darkblue")) +
  labs(title = "Spatial distribution of hake presence",color = "Presence")+
  guides(color = guide_legend(override.aes = list(size = 2)))

#Zero and non-zero trawls appears to be heavily clustered, suggesting a strong
#spatial correlation.


#From the graph that show survey locations across different years, it's easy to observe
#that not all the stations have been surveyed across different years:
ggplot(MEDITS_MERL_MOD_LOG, 
       aes(x = shooting_longitude, 
           y = shooting_latitude, 
           color = presence)) +
...
for (current_year in years) {
  ...
  # Subset the MEDITS_MERL_MOD_LOG  tibble for the year of interest
  year_data<-MEDITS_MERL_MOD_LOG %>% filter(year==current_year)
  
  # Convert coordinates to DD format
  year_data<-year_data %>% mutate(shooting_longitude_dd=ddmm.mmm_to_dd(shooting_longitude),
                                  shooting_latitude_dd=ddmm.mmm_to_dd(shooting_latitude))
  
  year_data <- year_data %>%
    distinct(shooting_longitude_dd, shooting_latitude_dd, .keep_all = TRUE)
  
  #Since we are interested in the coordinates only, let's select them:
  year_coords<-year_data %>% select(shooting_longitude_dd,shooting_latitude_dd)
  
  #We need to convert year_coords as a matrix to use the faction knearneigh:
  year_coords<-as.matrix(year_coords)
  
  neighbours<-knn2nb(knearneigh(year_coords, k = 10))
  lw<-nb2listw(neighbours)
  result<-moran.test(year_data$presence_num,lw)
  
  cat("Moran's I =", round(result$estimate[1], 4), 
...
for (current_year in years) {
  ...
  #Since we are interested in the coordinates only, let's select them:
  year_coords<-year_data %>% select(shooting_longitude_dd,shooting_latitude_dd,shooting_depth)
  
  #We need to convert year_coords as a matrix to use the faction knearneigh:
  year_coords<-as.matrix(year_coords)
  
  neighbours<-knn2nb(knearneigh(year_coords, k =50))
  lw<-nb2listw(neighbours)
  result<-moran.test(year_data$presence_num,lw)
...
for (current_year in years) {
  ...
  #Since we are interested in the coordinates only, let's select them:
  year_coords<-year_data %>% select(shooting_longitude_dd,shooting_latitude_dd,shootimg_depth)
  
  #We need to convert year_coords as a matrix to use the faction knearneigh:
  year_coords<-as.matrix(year_coords)
  
  neighbours<-knn2nb(knearneigh(year_coords, k = 100))
  lw<-nb2listw(neighbours)
  result<-moran.test(year_data$presence_num,lw)
...
#All the p values are extremly low, for each value of k, this means that 
#spatial autocorrelation is extremly high, and that it's one of the main
