...
#Several models fitted in this script (in particular the Negative Binomial    
#GAM and the two ZINB models) are extremely expensive to fit and may require  
#anywhere from several hours to a full day of computation depending on the    
#available hardware.                
                            
#Loading instructions are provided in the Modelling section.           
...
#IMPORTANT:
#This merging procedure is computationally expensive, as it iterates over all hauls.
#Execution time can be substantial (up to 30 minutes)
...
#This variable will not be included in our model as a covariate, since nbtot will
...
#R functions requires coordinates to be in decimal degrees so I convert the 
#shooting_latitude and shooting_longitude that are now stored in Degrees and Decimal 
#Minutes (DDMM.MMM) in decimal degrees (DD).
...
#indicate the presence of a missing value). 
#The share of missing /invalid data is too high so the variable should be removed:
...
#works as (Berg et al. 2024).
#https://doi.org/10.1016/j.fishres.2024.107108
...
#Level S has few observations inside, one option is to merge them in the other category:
...
#Let's convert the file to the tibble format:
...
#Digital Terrain Model, accessed via the EMODnet geoviewer(https://emodnet.ec.europa.eu/geoviewer/). 
#The 14 GeoTIFF tiles covering the study area (E4-E8, F4-F8, G5-G8) were downloaded,
#merged into a single raster using terra::merge(), and uploaded to Zenodo to 
#ensure full reproducibility and open access (Rossi, 2026; DOI: 10.5281/zenodo.19111208).
...
# - Flow dirextion: direction of steepest descent.
...
sum(MEDITS_MERL_MOD$valid_haul==FALSE) #299  observations should be removed, as 
#they violate the condition expressed above.
...
#Let's load the data necessary for modelling directly from GitHub (if the user doesn't 
#want to run the previous code)
tmp_rdata <- tempfile(fileext = ".RData")
download.file(
  url      = "https://raw.githubusercontent.com/DavideRossi123/Thesis-Project-6/main/MEDITS_data_for_modelling.RData",
  destfile = tmp_rdata,
  mode     = "wb"
)
load(tmp_rdata)
...
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~#
#OPTIONAL: Load pre-fitted models directly from GitHub #########################
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~#

#The models below (especially model_nb and the ZINB models) are extremely expensive
#to fit. Depending on your machine's computational power, fitting can take anywhere 
#from several hours up to a full day.
#To skip refitting, you can load the pre-fitted model objects directly from GitHub
#using the code below:


#Poisson GAM
tmp_pois <- tempfile(fileext = ".RDS")
download.file(
  url      = "https://raw.githubusercontent.com/DavideRossi123/Thesis-Project-6/main/model_pois.RDS",
  destfile = tmp_pois,
  mode     = "wb"
)
model_pois <- readRDS(tmp_pois)

#Negative Binomial GAM
tmp_nb <- tempfile(fileext = ".RDS")
download.file(
  url      = "https://raw.githubusercontent.com/DavideRossi123/Thesis-Project-6/main/model_nb.RDS",
  destfile = tmp_nb,
  mode     = "wb"
)
model_nb <- readRDS(tmp_nb)

#ZINB Model (final_results_zinb)
tmp_zinb <- tempfile(fileext = ".RDS")
download.file(
  url      = "https://raw.githubusercontent.com/DavideRossi123/Thesis-Project-6/main/final_results_zinb.RDS",
  destfile = tmp_zinb,
  mode     = "wb"
)
final_results_zinb <- readRDS(tmp_zinb)

#ZINB Model 2 (final_results_zinb2)
tmp_zinb2 <- tempfile(fileext = ".RDS")
download.file(
  url      = "https://raw.githubusercontent.com/DavideRossi123/Thesis-Project-6/main/final_results_zinb2.RDS",
  destfile = tmp_zinb2,
  mode     = "wb"
)
final_results_zinb2 <- readRDS(tmp_zinb2)
...
model_zi_init<- gam(zero_num ~ s(shooting_longitude_m,shooting_latitude_m, bs = "tp",k=150) +
                      s(shooting_depth, bs = "cr",k=30) +
                      s(bottom_temperature, bs = "cr") +
                      s(shooting_slope, bs="cr")+
                      folk_5, 
...
for(t in 1:k){
  ...
  model_zi <- gam(vi_t ~ s(shooting_longitude_m,shooting_latitude_m, bs = "tp",k=150) +
                    s(shooting_depth, bs = "cr",k=30) +
                    s(bottom_temperature, bs = "cr") +
                    s(shooting_slope, bs="cr")+
                    folk_5, 
  ...
  #Calculate the expected complete log-likelihood for convergence check
...
q_res_zinb <- q_res_zinb[is.finite(q_res_zinb)]
...
#Expert 1 and expert 2 initialization
...
model_expert2_init <- model_expert1_init
...
draw(final_results_zinb2$model_expert1,select=1)
...
#Partial smooth plots graphs of all the 4 models toghether #######################
...
#Spatial smooth plots of all 4 models together #################################

p_zi_spatial <- draw(final_results_zinb2$model_zi,      select = 1) +
  plot_annotation(
    title = "Zero-Inflation Gate",
    theme = theme(plot.title = element_text(size = 23, face = "bold", hjust = 0.5))
  )

p_mixing_spatial <- draw(final_results_zinb2$model_mixing, select = 1) +
  plot_annotation(
    title = "Mixing Gate",
    theme = theme(plot.title = element_text(size = 23, face = "bold", hjust = 0.5))
  )

p_expert1_spatial <- draw(final_results_zinb2$model_expert1, select = 1) +
  plot_annotation(
    title = "Expert 1",
    theme = theme(plot.title = element_text(size = 23, face = "bold", hjust = 0.5))
  )

p_expert2_spatial <- draw(final_results_zinb2$model_expert2, select = 1) +
  plot_annotation(
    title = "Expert 2",
    theme = theme(plot.title = element_text(size = 23, face = "bold", hjust = 0.5))
  )
...
# 2x2 patchwork
spatial_plot <- (wrap_elements(p_expert1_spatial) | wrap_elements(p_expert2_spatial)) /
  (wrap_elements(p_zi_spatial)      | wrap_elements(p_mixing_spatial))
...
#Parametric terms plot #########################################################

#Helper: extract termplot data as a data frame
get_term_data <- function(model, term) {
  tp <- termplot(model, terms = term, se = TRUE, plot = FALSE)
  df <- tp[[term]]
  df$term <- term
  return(df)
}

#Extract data for both experts
e1_month <- get_term_data(final_results_zinb2$model_expert1, "month")
e1_folk  <- get_term_data(final_results_zinb2$model_expert1, "folk_5")
e2_month <- get_term_data(final_results_zinb2$model_expert2, "month")
e2_folk  <- get_term_data(final_results_zinb2$model_expert2, "folk_5")

#Add line breaks to long folk_5 labels 
wrap_labels <- function(df) {
  df$x <- str_wrap(as.character(df$x), width = 12)
  df$x  <- factor(df$x, levels = unique(df$x))
  return(df)
}

e1_folk <- wrap_labels(e1_folk)  
e2_folk <- wrap_labels(e2_folk)  

#Compute shared y limits per variabl
month_limits <- range(c(e1_month$y - e1_month$se, e1_month$y + e1_month$se,
                        e2_month$y - e2_month$se, e2_month$y + e2_month$se))

folk_limits  <- range(c(e1_folk$y - e1_folk$se, e1_folk$y + e1_folk$se,
                        e2_folk$y - e2_folk$se, e2_folk$y + e2_folk$se))

#Plot function with optional fixed y scale
plot_term <- function(df, title, colour, ylims = NULL) {
  p <- ggplot(df, aes(x = x, y = y, ymin = y - se, ymax = y + se)) +
    geom_hline(yintercept = 0, linetype = "dashed", colour = "grey60") +
    geom_errorbar(width = 0.2, colour = colour, alpha = 0.5) +
    geom_point(size = 3, colour = colour) +
    geom_line(aes(group = 1), colour = colour, linewidth = 0.8) +
    labs(title = title, x = NULL, y = "Partial effect") +
...
#Build four panels with fixed y scale
p1 <- plot_term(e1_month, "Expert 1: Month",         "steelblue", month_limits)
p2 <- plot_term(e1_folk,  "Expert 1: Sediment Type", "steelblue", folk_limits)
p3 <- plot_term(e2_month, "Expert 2: Month",         "tomato",    month_limits)
p4 <- plot_term(e2_folk,  "Expert 2: Sediment Type", "tomato",    folk_limits)

#Plot
parametric_plot <- (p1 | p2) / (p3 | p4)
parametric_plot




#Randomised quantile residual diagnostic
...
#Let's define make_qq_fixed and make_hist_fixed to create residual plots with
#common axis to enhance comparability across the 3 models
...
training_plots <- (p1 | p2 | p3) /
  (p4 | p5 | p6) +
  plot_annotation(theme = theme(plot.title = element_text(size = 13, face = "bold",hjust = 0.5)))
training_plots


#Poisson randomized quantile residuals (training 2000–2020) ##################

mu_pois <- predict(model_pois, type = "response")
y       <- MEDITS_MERL_fit$nbtot

#Lower bound: F(y-1)
F_y_minus_1_pois <- ifelse(y == 0, 0,
                           ppois(y - 1, lambda = mu_pois))

#Upper bound: F(y)
F_y_pois <- ppois(y, lambda = mu_pois)

set.seed(2026)
u_pois <- runif(length(y), min = F_y_minus_1_pois, max = F_y_pois)
q_res_pois <- qnorm(u_pois)
q_res_pois <- q_res_pois[is.finite(q_res_pois)]
#Poisson training residual plots
all_pois_res <- q_res_pois
x_lim_pois <- range(all_pois_res, na.rm = TRUE)
y_lim_pois <- range(all_pois_res, na.rm = TRUE)

p_hist_pois <- make_hist_fixed(q_res_pois, "Poisson — Histogram (2000–2020)", x_lim_pois)
p_qq_pois   <- make_qq_fixed(q_res_pois,   "Poisson — Q-Q Plot (2000–2020)",  y_lim_pois)

pois_plots <- (p_hist_pois | p_qq_pois) +
  plot_annotation(
...
#quantile residual plots on validation data (2021)
...
#IMPORTANT:
#To retrieve temperature data, the user must have an active account on the
#Copernicus Marine Service and be logged in before running this code.
#Registration (free) is available at:
#https://marine.copernicus.eu/

#Authentication is required for the 'copernicusmarine' Python package to work.
#If the user is not logged in, the temperature extraction will fail.

#If you prefer not to set up Copernicus access, precomputed prediction maps
#are available here:
#https://github.com/DavideRossi123/Thesis-Project-6
...
#Filter points that are inside GSAs of interest
...
#Extract bathymetry on the prediction grid
...
#folk_5
#Let's extract the seabed substrate data
#Initialize EMODnet Geology WFS client
geo_wfs <- emodnet_init_wfs_client(
  service = "geology_seabed_substrate_maps")
...
#Let's build a function to allow user to select a certain prediction time and to
#then extract time-varying covariates accordingly
...
#Temperature extraction is handled by an external Python script called via
#reticulate, with automatic dependency installation if required.
...
# Get country polygons and crop to Mediterranean extent.
countries <- ne_countries(scale = 50, returnclass = "sf")
med_bbox <- st_bbox(med_gsas)
med_bbox_buffered <- med_bbox
med_bbox_buffered["xmin"] <- med_bbox["xmin"] - 1
med_bbox_buffered["xmax"] <- med_bbox["xmax"] + 1
med_bbox_buffered["ymin"] <- med_bbox["ymin"] - 1
med_bbox_buffered["ymax"] <- med_bbox["ymax"] + 1
countries_med <- st_crop(countries, med_bbox_buffered)
